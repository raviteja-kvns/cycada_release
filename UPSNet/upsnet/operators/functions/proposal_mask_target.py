...
class ProposalMaskTargetFunction(Function):
    @staticmethod
    def forward(ctx, rois, gt_boxes, gt_masks, num_classes, batch_images, batch_rois, fg_fraction, mask_size, binary_thresh):

        context = torch.device('cuda', rois.get_device())

        assert batch_rois == -1 or batch_rois % batch_images == 0, \
            'batchimages {} must devide batch_rois {}'.format(batch_images, batch_rois)
        ...
        rois, labels, bbox_targets, bbox_weights, mask_targets, mask_weights = \
            sample_rois(all_rois, fg_rois_per_image, rois_per_image, num_classes, gt_boxes=gt_boxes,
                        gt_masks=gt_masks, mask_size=mask_size, binary_thresh=binary_thresh)
        ...
        return torch.Tensor(rois).pin_memory().to(context, dtype=torch.float32, non_blocking=True), torch.Tensor(labels).pin_memory().to(context, dtype=torch.int64, non_blocking=True),\
            ...
            torch.Tensor(mask_targets).pin_memory().to(context, dtype=torch.float32, non_blocking=True), torch.Tensor(
