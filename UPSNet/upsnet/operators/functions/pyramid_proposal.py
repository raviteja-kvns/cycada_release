...
class PyramidProposalFunction(Function):
    @staticmethod
    def forward(ctx, cls_prob_p2, cls_prob_p3, cls_prob_p4, cls_prob_p5, cls_prob_p6,
                bbox_pred_p2, bbox_pred_p3, bbox_pred_p4, bbox_pred_p5, bbox_pred_p6, im_info, feat_stride, scales, ratios, rpn_pre_nms_top_n, rpn_post_nms_top_n, threshold, rpn_min_size, individual_proposals=False, batch_idx=0, use_softnms=False, crowd_gt_roi=None, num_anchors=3):
        device_id = cls_prob_p2.get_device()
        nms = gpu_nms_wrapper(threshold, device_id=device_id) if not use_softnms else soft_nms_wrapper(threshold)
        ...
        for s in range(len(feat_stride)):
            stride = int(feat_stride[s])
            sub_anchors = generate_anchors(stride=stride, sizes=scales * stride, aspect_ratios=ratios)
            ...
            keep = PyramidProposalFunction._filter_boxes(proposals, min_size * im_info[2])
            proposals = proposals[keep, :]
            scores = scores[keep]

            if crowd_gt_roi is not None:
                proposal_by_gt_overlap = bbox_overlaps(proposals, crowd_gt_roi * im_info[2])
        ...
        # batch inds are 0
        batch_inds = np.ones((proposals.shape[0], 1), dtype=np.float32) * batch_idx
