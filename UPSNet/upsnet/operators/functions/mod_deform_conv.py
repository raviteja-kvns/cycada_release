...
from .._ext.mod_deform_conv import mod_deform_conv_cuda


class ModDeformConvFunction(Function):
    @staticmethod
    def forward(ctx, data, offset, mask, weight, bias, in_channels, out_channels, kernel_size, stride,
        ...
        col_buffer = data.new().resize_(int(ctx.in_channels * np.prod(ctx.kernel_size)),
                                        ctx.output_shape[2], ctx.output_shape[3]).zero_()
        output = data.new().resize_(ctx.output_shape[0], ctx.output_shape[1],
                                    ctx.output_shape[2], ctx.output_shape[3]).zero_()
        ...
        for i in range(ctx.data_shape[0]):
            mod_deform_conv_cuda.mod_deform_im2col(data[i, :, :, :].contiguous(), offset[i, :, :, :].contiguous(), mask[i, :, :, :].contiguous(), ctx.data_shape,
                                                   tuple(col_buffer.size()
                                                         ), ctx.kernel_size, ctx.padding, ctx.stride, ctx.dilation,
    ...
    def backward(ctx, grad_output):
        ...
        if bias is not None:
            grad_bias = grad_output.data.sum(dim=0, keepdim=True).sum(
                dim=2, keepdim=True).sum(dim=3, keepdim=True).squeeze()
        else:
            grad_bias = None
        return Variable(grad_data), Variable(grad_offset), Variable(grad_mask), Variable(grad_weight), Variable(grad_bias) if grad_bias is not None else bias, \
    ...
    def shape_setup(ctx):
        ...
        ctx.output_shape = \
            (ctx.data_shape[0], ctx.out_channels,
             (ctx.data_shape[2] + 2 * ctx.padding[0] - ctx.dilation[0]
