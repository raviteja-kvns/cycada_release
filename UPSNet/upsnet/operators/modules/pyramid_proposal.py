...
class PyramidProposal(Module):
    ...
    def forward(self, cls_prob, bbox_pred, im_info, roidb=None):
        ...
        for i in range(im_info.shape[0]):
            if roidb is not None:
                crowd_gt_roi = roidb[i]['boxes'][np.where(
                    (roidb[i]['gt_classes'] > 0) & (roidb[i]['is_crowd'] == 1))[0], :]
                if crowd_gt_roi.size == 0:
                    crowd_gt_roi = None
            else:
                crowd_gt_roi = None
            rois_im_i, scores_im_i = \
                PyramidProposalFunction.apply(cls_prob[0][[i], :, :, :], cls_prob[1][[i], :, :, :],
                                              cls_prob[2][[i], :, :, :], cls_prob[3][[i], :, :, :],
                                              cls_prob[4][[i], :, :, :], bbox_pred[0][[i], :, :, :],
                                              bbox_pred[1][[i], :, :, :], bbox_pred[2][[i], :, :, :],
                                              bbox_pred[3][[i], :, :, :], bbox_pred[4][[i], :, :, :],
                                              torch.from_numpy(im_info[i, :]),
                                              self.feat_stride, self.scales, self.ratios, self.rpn_pre_nms_top_n,
