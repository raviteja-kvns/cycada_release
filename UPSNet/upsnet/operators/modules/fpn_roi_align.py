...
class FPNRoIAlign(Module):
    ...
    def forward(self, feat, rois):
        ...
        rois_index = torch.tensor(np.argsort(np.hstack(feat_no))
                                  [-rois.shape[0]:], dtype=torch.int64, requires_grad=False).to(context)
        ...
        for i in range(len(self.spatial_scale)):
            ...
            pool_feat.append(
                RoIAlignFunction.apply(feat[i], rois_fpn[i], self.pooled_height, self.pooled_width, self.spatial_scale[i]))
